...
(function($){	
	...
	var _jQuizMeLayout = $("<div/>").addClass( "quiz-el").append(
		$("<div/>").addClass( "q-header q-innerArea").append(
			...
			//$("<div/>").addClass( "q-help q-innerArea").append( changed this
			$("<div/>").addClass( "q-help").append( 
			$("<div/>").addClass( "q-help-menu" ).append(
				//hide modal help modal box by commenting out code
				//$("<input type='button' data-toggle='modal' data-target='#myModal'/>").addClass( "q-help-btn" ),									
		...
		$("<div/>").addClass( "q-review-menu q-innerArea").append( 
			...
			$("<div/>").addClass( "q-reviewBar q-innerArea").append(
				$("<input type='button' title='click to go to previous question'/>").attr({ "class": "q-leftArrow q-review-arrows", "value": "<" }),
				$("<input type='button' title='click to go to next question'/>").attr({ "class": "q-rightArrow q-review-arrows", "value": ">" }),
				 $( "<span/>" ).addClass( "q-review-nav" ).append(
					$("<div/>").addClass( "styled-select blue").append( $("<select/>").addClass( "q-review-index-all q-review-index") ),
					$("<div/>").addClass( "styled-select-missed blue").append( $("<select/>").addClass( "q-review-index-missed q-review-index").hide() ).hide(),
					//$("<span/>").addClass( "q-missMarker" ).show(),
					$("<div/>").append(
					//$("<input type='checkbox' id='checkboxG1'/>").addClass( "q-showOnlyMissed-btn"), // css-checkbox
					//$("<label for='checkboxG1' title='Click to show only missed questions.'/>").addClass( "css-label")
					)
				)
			).hide()
		),
		$("<div/>").addClass( "q-intro q-innerArea" ).append(
			...
			$( '<input type="button" title="Click to start activity"/>' ).addClass( "q-begin-btn center-block" )
		).hide(),
		$("<div/>").addClass( "q-prob q-innerArea" ).append( 
			...
			$("<div/>").addClass( "q-ans q-probArea").append(
				...
				$("<div/>").addClass( "counter-container").append(
	...
	_lang = {
			ans:{	// _lang.ans are shown during the display of .q-result, the answer result.
				corrAns: "Your answer is incorrect.<br/>This is the correct answer:",
				praise: 'Correct answer. Good job!',
	...
	$.fn.jQuizMe = function( wordList, options, userLang ){
		...
		return this.each( function(){
			...
			disableMenuBar = function(){
				...
				$( ".q-quit-btn, .q-help-btn", currQuiz ).hide();
			...
			setCheckAndNextBtn = function(){
				...
				$( ".q-check-btn", currQuiz ).attr( "disabled", true )
					.click( function( e ){
						...
						$('.q-next-btn').show();
						$('.q-check-btn').hide();
						postDataJSON.numOfRight = stats.numOfRight;
                        postDataJSON.numOfWrong = stats.numOfWrong;
                        postDataJSON.quesTried = stats.quesTried;
                        postDataJSON.totalQues = stats.totalQues;
                        postDataJSON.numOfQues = stats.numOfQues;
                        postDataJSON.quizName = options.quizName;
                        postDataJSON.rate = stats.accurTxt();
                        postDataJSON.total = stats.percTxt();
						saveSkillsActivity(postDataJSON);
					}
				);
			...
			setQuitBtn = function(){
				...
				//	$( ".q-quit-btn", currQuiz).hide();
				//});
				$(".q-quitNo-btn", currQuiz).click(function() {
			...
			setReviewMenu = function(){
				...
				$( ".q-reviewBar-btns", currQuiz ).click( function( e ){
					...
					animateThis( hideEl, 0, function(){
						console.log("changing focus of buttons");
						if(isDetailEl) {
							$( ".q-review-btn", currQuiz ).addClass( "lighter-blue");
							$( ".q-details-btn", currQuiz ).removeClass( "lighter-blue");
						}
						if(!isDetailEl) {
							$( ".q-details-btn", currQuiz ).addClass( "lighter-blue");
							$( ".q-review-btn", currQuiz ).removeClass( "lighter-blue");
						}
				...
				$( ".q-showOnlyMissed-btn", currQuiz ).change( function(){
					...
					if( ckd ){
						...
						$( ".styled-select-missed", currQuiz ).fadeIn().css( "display", "inline-block" );
						$( ".q-review-index-all", currQuiz ).hide();
						$( ".styled-select", currQuiz ).hide();
					}
					else{
						$( ".q-review-index-missed", currQuiz ).hide();
						$( ".styled-select-missed", currQuiz ).hide();
						$( ".q-review-index-all", currQuiz ).fadeIn().css( "display", "inline" );
						$( ".styled-select", currQuiz ).fadeIn();
					}
			...
			setupReview = function(){
				...
				$('.q-review-btn').addClass('lighter-blue');
				$( ".q-review-menu", currQuiz).show();
				$( ".counter-container", currQuiz).hide();
				...
				if( settings.intro ){
					...
					$( ".q-begin-btn", currQuiz ).unbind().one( "click", function(){
						animateThis( $( ".q-intro", currQuiz ), 0, function(){ 
							...
							$( ".counter-container", currQuiz).show();
						});
					});
				}			
			},
			...
			getUserAnsForDisplay = function( toUni ){
				...
				return ( !getProblemProp( "userAnswer" ) ) ? "<span>" + ans +"</span>" :
